...
app.config['SQLALCHEMY_DATABASE_URI'] = f'postgresql://{USER}:{PSW}@{HOST}:5123/{DB}'
...
def get_all_ads(current_user):
    advts = Advt.query.filter_by(user_id=current_user.id).all()     
    output = []
    for advt in advts:
        advt_data = {}
...
def get_one_ad(current_user, advt_id):
    advt = Advt.query.filter_by(id=advt_id, user_id=current_user.id).first()
    if not advt:
...
def create_advt(current_user):
    data = request.get_json()
    new_advt = Advt(text=data['text'], title=data['title'], user_id=current_user.id)
